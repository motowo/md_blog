...
const SalesManagement: React.FC = () => {
  ...
  const fetchSales = async () => {
    try {
      setLoading(true);
      const response = await salesApi.getSales(selectedMonth);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">売上管理</h1>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card>
            <CardBody>
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400">総売上</div>
          ...
          <Card>
            <CardBody>
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400">販売数</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
          ...
          <Card>
            <CardBody>
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400">手数料</div>
          ...
          <Card>
            <CardBody>
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400">純売上</div>
      ...
      <Card>
        <CardHeader>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">売上履歴</h2>
        </CardHeader>
        <CardBody>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {selectedMonth ? '選択された月の売上はありません' : '有料記事の売上がまだありません'}
              </p>
            </div>
          ) : (
                  ...
                  {sales.map((sale) => (
                    <tr key={sale.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">
                        <div className="max-w-xs truncate">{sale.article_title}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                        {sale.user.name}
                      </td>
