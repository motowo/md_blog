...
### ✨ 最新: 公開プロフィール機能強化の実装完了
#### 🎯 機能強化
- **記事ナビゲーション**: 記事タイトルクリックで記事詳細ページに遷移
- **ページネーション機能**: 10件ごとの記事表示、先頭・末尾・前後ボタン付き
- **ダブルページネーション**: 記事一覧の上部・下部に両方配置
- **記事表示範囲**: 総記事数と現在表示範囲の明示（例：30件中 1-10件を表示）
#### 🔧 TypeScript型定義強化
- **Article型拡張**: `excerpt`と`payments_count`フィールド追加
- **Badge関数修正**: `getBadgeClass`の引数を正しく修正
#### 🐛 UTF-8エンコーディングエラー修正
- **根本原因解決**: ArticleControllerでのJSON生成時エラーを修正
- **データクリーニング**: `cleanUtf8()`メソッド実装、`mb_convert_encoding()`による文字正規化
- **全レスポンス対応**: 未ログイン、作成者、購入済み、無料記事すべてのケースでクリーニング適用
- **安全なシリアライゼーション**: 無効なUTF-8文字による500エラーを根絶
