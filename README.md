...
- **プロフィール表示機能**: 公開プロフィール表示、記事からの著者プロフィール遷移、プライバシー設定対応
- **管理者機能**: ダッシュボード、ユーザー管理、記事管理、振込管理、手数料設定
- **UI統一化**: Badge色統一、コンポーネントスタイル標準化、ユーザー名表示統一
- **クレジットカード管理**: ユーザーごと1枚の登録・管理機能
- **金額表示統一**: 共通部品化による一貫した整数表示（/src/utils/currency.ts）
- **売上・振込管理**: JST基準の正確なタイムゾーン処理による売上集計・振込管理機能
- **手数料システム**: 動的手数料設定、期間別適用、自動計算機能
- **著者振込管理**: 月次サマリ表示、支払いステータス管理、重複処理防止
- **管理者制限**: 無料記事専用投稿、プロフィール項目制限、ヒートマップ非表示
- **トップページカルーセル**: 新着記事・注目記事を3件表示、1件ずつスライド、自動再生、視認性に優れたUI
...
- **有料/無料記事比率**: 44.4%/55.6%（有料1,812記事、無料2,273記事）
- **価格範囲**: 300-1,250円（50円単位）
- **ユーザー数**: 30名（超アクティブ6名、中程度18名、無料専門3名、投稿なし3名）
...
**ログイン状態維持機能:**
- ログイン時に「ログイン状態を保持する」チェックボックスが利用可能
...
- `GET /api/articles/recent` - 新着記事取得（作成日時順、デフォルト10件）
- `GET /api/articles/trending` - 注目記事取得（過去1ヶ月売上順、デフォルト10件）
